...
# from gen_dataset import iir_design, get_mel_scale, bandpass_filter_iir
...
def my_accuracy(y_true, y_pred):
    print(K.mean(2*K.abs(y_true-0.5) * K.equal(y_true, K.round(y_pred)), axis=-1))
...
def train(x_train, y_train, vad_train, batch_size=64, epochs=10, model_name="model.h5"):
    ...
    history = model.fit(x_train, [y_train, vad_train],
                        batch_size=timestamp_size, epochs=epochs, verbose=2, shuffle=False,
